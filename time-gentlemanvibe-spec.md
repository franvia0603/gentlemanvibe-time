...
- 목적: 일반(비풀스크린) 화면에서 방문자가 "전체화면으로 써야 진짜 가치가 나온다"는 걸 바로 알게 한다.
- 위치: 각 기능(클락/월드클락/타이머/알람/스톱워치/뽀모도로) 화면 하단, 튀지 않는 작은 안내 문구.
- 카피 템플릿: "무료 [기능명] 사용법: 탭이나 폰에서 전체화면 버튼을 누르시면 깔끔한 GV [기능명] 모드로 나만의 데스크테리어를 완성시켜드립니다."
- 동작: 문구 클릭 시 Fullscreen API로 바로 풀스크린 진입. 이미 풀스크린 상태면 렌더링 안 함.
- 공통 컴포넌트(FullscreenHint)로 구현, 기능마다 이름만 교체해 재사용 — 이미 구현 완료.

### 3.5 메인 클락 표시 옵션 (Display Settings)
- 오전/오후(AM/PM) 표시 토글 (끄면 24시간제)
- 시간만 표시 토글 (초 숨김, 시:분만 크게)
- 날짜·요일 표시 토글
- 날씨 표시(위치 기반, Geolocation API + Open-Meteo API — 키 불필요 무료) 토글, 권한 거부 시 조용히 숨김
- 각 옵션 localStorage 저장
- 설정 아이콘 클릭 시 작은 패널로 토글 노출, 풀스크린에서는 숨김

### 3.6 도구 페이지 콘텐츠 & 광고 자리 (vClock.kr 구조 참고)
- 각 도구 화면 하단에 "[도구명] 사용법" h1 + 300~500자 설명 문단
- 도구 패널과 설명 섹션 사이에 광고 삽입용 빈 컨테이너(placeholder) 미리 확보
- 공유 버튼, embed 기능, 대량 SEO 랜딩 페이지는 채택하지 않음
...
- **타이포그래피**: UI/라벨 텍스트는 Inter / Manrope 등 휴머니스트 산세리프, 얇은 굵기 + 넓은 트래킹의 tabular-nums 사용. 클락·타이머 등 큰 숫자 표시는 **Bebas Neue**로 확정 (세븐세그먼트/디지털 계산기 폰트는 지양하되, 브랜드 아이덴티티를 위한 굵고 콘덴스드한 디스플레이 서체는 채택). 라벨 텍스트에 세리프 소량 배합 가능 (클래식함 보강).
...
물리적 원형 다이얼 타이머(Time Timer류)를 웹 UI로 재해석:
- 원형 다이얼에 0/5/10/15.../55분 눈금 표시
- 다이얼 안쪽 방사형 세그먼트 링, 경과 시간만큼 앰버 톤으로 채워짐
- 원 중심에 Bebas Neue 폰트로 남은 시간(MM:SS) 크게 표시
- 배경은 매트 블랙/차콜, 물리적 베젤·버튼 같은 하드웨어 디테일은 재현하지 않음
- 세그먼트 진행색은 gv-amber/gv-amber-glow, 눈금·숫자는 gv-titanium
